...
# from recbole.utils.cython.random_choice import randint_choice
...
# 自定义实现randint_choice函数，替代原本从recbole.utils.cython导入的版本
def randint_choice(high, size=1, replace=True, p=None, exclusion=None):
    """Sample random integers from [0, high).
    
    Args:
        high (int): The upper bound (exclusive) for the random integers.
        size (int): The number of random integers to sample.
        replace (bool): Whether to sample with replacement.
        p (array-like): Probability distribution (not implemented).
        exclusion (set): A set of integers to exclude from sampling.
        
    Returns:
        int or list: A random integer or a list of random integers.
    """
    if size <= 0:
    ...
    if p is not None:
        raise NotImplementedError("Sampling with probability distribution is not implemented.")

    # 转换exclusion为集合（如果不是None）
    if exclusion is not None:
        exclusion = set(exclusion)
        if high <= len(exclusion):
    ...
    # 如果不需要排除任何数字，使用numpy的随机采样
    if exclusion is None:
        if replace:
            result = np.random.randint(0, high, size=size)
        else:
            result = np.random.choice(high, size=size, replace=False)
    else:
        # 创建一个排除了exclusion的候选数字列表
        candidates = [i for i in range(high) if i not in exclusion]
        if replace:
            result = np.random.choice(candidates, size=size, replace=True)
        else:
            result = np.random.choice(candidates, size=size, replace=False)
...
class SUPCCL(GeneralRecommender):
    ...
    def calculate_loss(self, interaction, epoch=None, batch_idx=None, item_clusters=None):
